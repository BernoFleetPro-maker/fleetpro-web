      ...
      <div className="max-w-5xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-slate-400 text-sm">Manage tenants, controllers, and clients across the whole platform</p>
          </div>
          <button
            onClick={onLogout}
        ...
        <div className="flex gap-2 mb-6 border-b border-slate-700">
          {["tenants", "controllers", "clients"].map(tab => (
            <button
              key={tab}
        ...
        {loading ? (
          ...
          <>
            {activeTab === "tenants" && (
              <TenantsTab tenants={tenants} authHeaders={authHeaders} reload={loadAll} />
            )}
            {activeTab === "controllers" && (
              <ControllersTab controllers={controllers} tenants={tenants} authHeaders={authHeaders} reload={loadAll} />
            )}
            {activeTab === "clients" && (
              <ClientsTab clients={clients} tenants={tenants} authHeaders={authHeaders} reload={loadAll} />
            )}
          </>
        )}
      </div>
...
// ── TENANTS TAB ───────────────────────────────────────────────────────────────
function TenantsTab({ tenants, authHeaders, reload }) {
...
// ── CONTROLLERS TAB ───────────────────────────────────────────────────────────
function ControllersTab({ controllers, tenants, authHeaders, reload }) {
  const [name, setName] = useState("");
  ...
  async function createController() {
    setError("");
    if (!name || !username || !password || !tenantId) {
      setError("All fields are required");
      return;
    }
    try {
      await api.post("/superadmin/controllers", { name, username, password, tenantId }, authHeaders);
      setName(""); setUsername(""); setPassword(""); setTenantId("");
      reload();
    } catch (err) {
  ...
  return (
    <div>
      <div className={cardClass}>
        <h3 className="text-sm font-semibold mb-3">Create new controller login</h3>
        <div className="flex gap-2 mb-3">
        ...
        <div className="flex gap-2 mb-3">
          <select className={fieldClass} value={tenantId} onChange={e => setTenantId(e.target.value)}>
        ...
        <tbody>
          {controllers.map(c => (
            <tr key={c.id} className="border-b border-slate-800">
              ...
              <td className="py-2">{c.tenant?.displayName || "—"}</td>
              <td className="py-2">
                <button
...
// ── CLIENTS TAB ────────────────────────────────────────────────────────────────
function ClientsTab({ clients, tenants, authHeaders, reload }) {
  const [name, setName] = useState("");
  const [username, setUsername] = useState("");
  const [password, setPassword] = useState("");
  const [tenantId, setTenantId] = useState("");
  ...
  async function createClient() {
    ...
    try {
      await api.post("/superadmin/clients", { name, username, password, tenantId, permission }, authHeaders);
      setName(""); setUsername(""); setPassword(""); setTenantId(""); setPermission("view");
      reload();
    } catch (err) {
  ...
  return (
    <div>
      <div className={cardClass}>
        <h3 className="text-sm font-semibold mb-3">Create new client login</h3>
        <div className="flex gap-2 mb-3">
