    ...
    <div className="flex h-full" style={{ minHeight: "calc(100vh - 0px)" }}>
      ...
      <div className="w-80 flex-shrink-0 bg-white border-r flex flex-col overflow-hidden">
        <div className="p-4 border-b">
          <h2 className="text-lg font-bold text-gray-800">Dropoff Points</h2>
        ...
        <div className="p-4 border-b bg-gray-50">
          <form onSubmit={handleSubmit} className="space-y-2">
            ...
            {/* Pin status */}
            <div className={`text-xs px-2 py-1.5 rounded ${pinned ? "bg-green-50 text-green-700 border border-blue-200" : "bg-gray-100 text-gray-400"}`}>
              {pinned
            ...
            <div className="flex gap-2 pt-1">
              <button type="submit" disabled={saving}
                className="flex-1 bg-green-600 hover:bg-green-700 disabled:bg-green-400 text-white py-2 rounded text-sm font-medium">
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {!loading && points.length === 0 && (
            <p className="text-gray-400 text-sm p-4">No dropoff points yet. Click the map to add one.</p>
          )}
          {points.map((p) => (
            <div key={p.id}
              className="px-4 py-3 border-b hover:bg-green-50 cursor-pointer"
              onClick={() => startEdit(p)}
            >
